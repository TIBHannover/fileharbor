...
from analyser.utils import get_features_from_db_entry
from analyser.utils import image_from_proto, dict_from_proto
...
class Searcher:
    ...
    def merge_feature(self, features):
        ...
        for p in features:
            ...
            plugin_feature[plugin][type].append(
                {"value": np.asarray(value), "weight": weight}
            )
        ...
        for p, plugin_data in plugin_feature.items():
            for t, values in plugin_data.items():
                mean_value = np.mean(
                    np.stack([v["value"] * v["weight"] for v in values]), axis=0
                )
    ...
    def parse_query(self, query):
        try:
            ...
            if query.sorting == analyser_pb2.SearchRequest.SORTING_CLASSIFIER:
                sorting = "classifier"
            if query.sorting == analyser_pb2.SearchRequest.SORTING_FEATURE:
                sorting = "feature"
            if query.sorting == analyser_pb2.SearchRequest.SORTING_RANDOM:
                sorting = "random"
            if query.sorting == analyser_pb2.SearchRequest.SORTING_RANDOM_FEATURE:
            ...
            if query.mapping == analyser_pb2.SearchRequest.MAPPING_UMAP:
                mapping = "umap"
            ...
            if query.clustering == analyser_pb2.SearchRequest.CLUSTERING_GM:
                clustering = "gm"
            if query.clustering == analyser_pb2.SearchRequest.CLUSTERING_KMEANS:
                clustering = "kmeans"
            if query.clustering == analyser_pb2.SearchRequest.CLUSTERING_AGGLOMERATIVE:
            ...
            for extra in query.extras:
                if extra == analyser_pb2.SearchRequest.EXTRA_FEATURES:
            ...
            for term in query.terms:
                ...
                if term_type == "text":
                    ...
                    if text.flag == analyser_pb2.TextSearchTerm.MUST:
                        flag = "must"
                    if text.flag == analyser_pb2.TextSearchTerm.SHOULD:
                        flag = "should"
                    if text.flag == analyser_pb2.TextSearchTerm.NOT:
                        flag = "not"

                    text_search.append(
                        {"field": field, "query": text.query, "flag": flag}
                    )
                ...
                if term_type == "number":
                    ...
                    # parse relation
                    relation = "eq"
                    if number.relation == analyser_pb2.NumberSearchTerm.GREATER:
                        relation = "gt"
                    if number.relation == analyser_pb2.NumberSearchTerm.GREATER_EQ:
                        relation = "gte"
                    if number.relation == analyser_pb2.NumberSearchTerm.EQ:
                        relation = "eq"
                    if number.relation == analyser_pb2.NumberSearchTerm.LESS_EQ:
                        relation = "lte"
                    if number.relation == analyser_pb2.NumberSearchTerm.LESS:
                        relation = "lt"

                    flag = "should"
                    if number.flag == analyser_pb2.NumberSearchTerm.MUST:
                        flag = "must"
                    if number.flag == analyser_pb2.NumberSearchTerm.SHOULD:
                        flag = "should"
                    if number.flag == analyser_pb2.NumberSearchTerm.NOT:
                        flag = "not"
                ...
                if term_type == "image_text":
                    ...
                    feature_results = list(
                        self.image_text_plugin_manager.run([image_text.query])
                    )[0]

                    if image_text.flag == analyser_pb2.ImageTextSearchTerm.NEGATIVE:
                        weight_mult = -1.0
                    else:
                        weight_mult = 1.0
                ...
                if term_type == "feature":
                    ...
                    if feature.flag == analyser_pb2.FeatureSearchTerm.NEGATIVE:
                        weight_mult = -1.0
                    else:
                        weight_mult = 1.0
    ...
    def build_search_body(
            ...
            if "field" not in e or e["field"] in [None, ""]:
                ...
                if len(field_path) == 1:
                    if field_path[0] == "meta":
                        ...
                        term = Q(
                            "multi_match", fields=["classifier_text"], query=e["query"]
                        )
                    elif field_path[0] == "origin":
                        term = Q(
                            "multi_match", fields=["origin_text"], query=e["query"]
                        )
            ...
            if "field" not in e or e["field"] is None:
                ...
                if len(field_path) == 2:
                    ...
                    if field_path[0] == "meta":
                        ...
                        if e["relation"] != "eq":
                            value_match = Q(
                                "range", meta__value_int={e["relation"]: e["query"]}
                            )
                        else:
                        ...
                        if e["relation"] != "eq":
                            value_match = Q(
                                "range", origin__value_int={e["relation"]: e["query"]}
                            )
                        else:
        ...
        if isinstance(sorting, str) and sorting.lower() == "random":
            should_terms.append(
                Q("function_score", functions=[{"random_score": {"seed": seed}}])
            )
    ...
    def __call__(self, query: Dict):
        ...
        if len(entries) <= 1:
            ...
            entries_padding = self.search_db(
                body=body_padding, size=max(len(whitelist_padding), 500)
            )
        ...
        if query["sorting"] == "feature":
            entries = list(
                self.mapping_plugin_manager.run(
                    entries, query["feature_search"], ["FeatureCosineMapping"]
                )
            )
