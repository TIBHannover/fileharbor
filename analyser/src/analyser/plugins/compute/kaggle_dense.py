...
class KaggleDenseClassifier(ComputePlugin):
    default_config = {
        ...
        "model_device": "gpu",
        "model_file": "kaggle_densenet.pb",
        "mapping_file": "kaggle_mapping.json",
        "multicrop": True,
        "max_dim": None,
        "min_dim": 244,
        "threshold": 0.5,
    ...
    def __init__(self, **kwargs):
        ...
        self.genre_lookup = sorted(
            [(token, index) for token, index in self.mapping["genres"].items()],
            key=lambda x: x[1],
        )

        self.style_lookup = sorted(
            [(token, index) for token, index in self.mapping["styles"].items()],
            key=lambda x: x[1],
        )
    ...
    def call(self, entries):
        ...
        for entry in entries:
            ...
            result = self.con.modelrun(
                self.model_name,
                f"image_{job_id}",
                [f"genre_probabilities_{job_id}", f"style_probabilities_{job_id}"],
            )
            ...
            if genre_prob > self.threshold:
                concepts.append(
                    analyser_pb2.Concept(
                        concept=genre[0], type="genre", prob=genre_prob
                    )
                )
            ...
            if style_prob > self.threshold:
                concepts.append(
                    analyser_pb2.Concept(
                        concept=style[0], type="style", prob=style_prob
                    )
                )
