    ...
    def seq2str(self, seqs):
        ...
        for seq in seqs:
            ...
            for i_lev in range(len(seq)):
                if seq[i_lev] != 0:
                    label_hirarchy.append(
                        self.classifier_config[i_lev]["tokenizer"][seq[i_lev]]
                    )
    ...
    def call(self, entries):
        ...
        for entry in entries:
            ...
            result = self.con.modelrun(
                self.model_name,
                f"image_{job_id}",
                [f"seqs_{job_id}", f"probs_{job_id}"],
            )
            ...
            seqs = [
                {"seq": self.seq2str([s[1:]])[0], "prob": np.exp(probabilities[i])}
                for i, s in enumerate(seqs)
            ]

            seqs = [
                {**x, "meta": self.mapping[x["seq"]]}
                for x in seqs
                if x["seq"] in self.mapping
            ]
            ...
            seqs = [
                ...
                if all(j["seq"] not in k["seq"] for k in seqs[i + 1 :])
            ]
            ...
            for x in seqs:
                ...
                concepts.append(
                    analyser_pb2.Concept(
                        concept=name, type="concept", prob=x["prob"].item()
                    )
                )
